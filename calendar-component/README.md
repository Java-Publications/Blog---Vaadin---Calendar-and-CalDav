...
[`calendar-caldav`](../calendar-caldav), and presents a fully-featured
...
Brings `calendar-caldav` (headless wire + service) as a transitive
plus `com.vaadin:vaadin-core`, `org.vaadin.stefan:fullcalendar2`, and
the bundled `META-INF/resources/frontend/styles/calendar-view.css`.
...
- **Translation keys** — every dialog/button takes its label from
  ...
  set of `K_*` constants on `CalendarView` and the sub-components in
  `com.svenruppert.vaadin.calendar.ui`.
- **Styling** — class names follow `.calendar-view__*` and
  `.calendar-nav__*` BEM-ish patterns. The bundled CSS in
  `META-INF/resources/frontend/styles/calendar-view.css` is fully
